...
const AiChatInterface = () => {
  ...
  const handleGenerateMessage = async () => {
    ...
    try {
      const modelsToProcess = generateImageEnabled
        ? aiModels.filter(m => ["gemini", "deepseek"].includes(m.id))
        : aiModels.filter(m => !lockedModels[m.id]);

      await Promise.all(
        modelsToProcess.map(async (model) => {
          ...
          console.log(`${isImageMode ? "Image" : "Text"} Generation Endpoint:`, endpoint);
          console.log("Prompt:", inputMessage);
          ...
          if (isImageMode) {
            ...
            console.log("this is the output : ", data?.content)
          }
        })
      );
