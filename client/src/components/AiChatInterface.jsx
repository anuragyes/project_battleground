...
//   //     setSpeakingMessages({ [msgId]: true });
//   //   } else {
//   //     window.speechSynthesis.cancel();
//   //     setSpeakingMessages((prev) => ({ ...prev, [msgId]: false }));
//   //   }
...
//     // Remove unwanted symbols, keep numbers and letters
//     return text.replace(/[^a-zA-Z0-9\u0900-\u097F\s.]/g, "");
//     // includes Hindi unicode range (0900–097F)
//   };
...
//                     <button  className="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:from-purple-700 hover:to-indigo-700 transition">
//                       Upgrade Now
//                     </button>
//                   </div>
//                 )}
...
const AiChatInterface = () => {
  ...
  const cleanTextForTTS = (text) => text.replace(/[^a-zA-Z0-9\u0900-\u097F\s.]/g, "");
  ...
  const handleGenerateMessage = async () => {
    ...
    try {
      if (generateImageEnabled) {
        // IMAGE generation for Gemini and DeepSeek
        await Promise.all(aiModels
          .filter(m => ["gemini", "deepseek"].includes(m.id))
          .map(async (model) => {
            const endpoint = model.id === "gemini"
              ? "http://localhost:5000/api/genterateimg/generate-img"
              : "http://localhost:5000/api/generate_deep_image/generate_image";

            const { data } = await axios.post(endpoint, { prompt: inputMessage }, {
              headers: { Authorization: `Bearer ${token}` }
            });
            ...
            if (data.success) {
              newConvos[model.id] = [
                ...newConvos[model.id],
                { id: Date.now() + Math.random(), sender: "user", text: inputMessage },
                { id: Date.now() + Math.random(), sender: "ai", image: data.url || data.content, ai: model.name },
              ];
            } else {
              toast.error(`${model.name} image generation failed`);
            }
          })
        );
      } else {
        // TEXT generation for all unlocked models
        await Promise.all(aiModels
          .filter(m => !lockedModels[m.id])
          .map(async (model) => {
            const { data } = await axios.post(model.endpoint, { prompt: inputMessage }, {
              headers: { Authorization: `Bearer ${token}` }
            });
            ...
            newConvos[model.id] = [
              ...newConvos[model.id],
              { id: Date.now() + Math.random(), sender: "user", text: inputMessage },
              { id: Date.now() + Math.random(), sender: "ai", text: data?.content || "No response", ai: model.name },
            ];
          })
        );
      }
                      ...
                      <div className={`max-w-xs rounded-lg p-3 text-sm relative ${msg.sender === "user" ? "bg-blue-600 text-white" : msg.isError ? "bg-red-100 text-red-700" : msg.isUpgrade ? "bg-gradient-to-r from-blue-500 to-purple-500 text-white flex flex-col items-center justify-center p-6 text-center" : "bg-gray-200 text-gray-800"}`}>
                        {msg.isUpgrade ? (
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex-1"><Markdown>{msg.text}</Markdown></div>
                            <button onClick={() => handleCopyText(msg.text)} className="ml-2 text-gray-500 hover:text-gray-900" title="Copy">📋</button>
                            <button onClick={() => toggleSpeak(msg.id, msg.text)} className="ml-2 text-gray-500 hover:text-gray-900">
                              {speakingMessages[msg.id] ? "🛑" : "🔊"}
                            </button>
                          </div>
                        )}
          ...
          <div className="flex flex-col sm:flex-row items-center bg-gray-100 border border-gray-300 rounded-full px-3 py-2 shadow gap-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={!inputMessage.trim() || imageLoading}
                className={`flex items-center justify-center h-10 w-10 rounded-full transition ${!inputMessage.trim() || imageLoading ? "bg-gray-400 text-gray-200 cursor-not-allowed" : "bg-green-600 text-white hover:bg-green-700"}`}
                title={generateImageEnabled ? "Generate Image" : "Send"}
              >
                {imageLoading ? (
                  <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
